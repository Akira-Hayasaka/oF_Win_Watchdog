...
void ofApp::update() 
{
	...
	while (receiver.hasWaitingMessages())
	{
		...
		if (m.getAddress() == "/kill_watchdog")
		{
			ofLogNotice(ofGetTimestampString("%Y.%m.%d.%H:%M.%S")) << "kill_watchdog";
			ofExit();
		}

		if (m.getAddress() == "/kill_tgt_app")
		{
			ofLogNotice(ofGetTimestampString("%Y.%m.%d.%H:%M.%S")) << "/kill_tgt_app";
			terminate_app(name_to_kill);
			b_manual_boot = true;
		}

		if (m.getAddress() == "/start_tgt_app")
		{
		...
		if (m.getAddress() == "/reboot_machine")
			reboot_machine();
		if (m.getAddress() == "/poweroff_machine")
			poweroff_machine();
	}
...
void ofApp::poweroff_machine()
{
	ofLogNotice(ofGetTimestampString("%Y.%m.%d.%H:%M.%S")) << "force poweroff machine";
	...
	if (OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken) == FALSE)
		ofLogFatalError("poweroff machine") << "fail to open process token";

	if (LookupPrivilegeValue(NULL, SE_SHUTDOWN_NAME, &TokenPri.Privileges[0].Luid) == FALSE)
		ofLogFatalError("poweroff machine") << "fail to get LUID";
